  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch followers
        const followersResponse = await fetch('/api/get-followers', {
          headers: {
      ...
      <div className="flex">
        ...
        <main className="flex-1 max-w-2xl mx-auto p-4">
          <div className="bg-white rounded-lg shadow p-6">
            <h1 className="text-2xl font-bold mb-4 text-gray-700">My Profile</h1>
            <h2 className="text-xl font-semibold mb-4 text-gray-700">Followers ({followers.length})</h2>
            {followers.length === 0 ? (
                ...
                {followers.map((follower) => (
                  <div key={follower.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="font-medium text-gray-700">{follower.username}</span>
                    </div>
                    <Button
                      onClick={() => handleFollow(follower.id)}
                      variant={following.includes(follower.id) ? "secondary" : "primary"}
                      size="sm"
                    >
